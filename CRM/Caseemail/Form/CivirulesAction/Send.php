 ...
 */
class CRM_Caseemail_Form_CivirulesAction_Send extends CRM_Core_Form {
  ...
  protected function getCaseRoles() {
    $cRoles = array();
    try {
  ...
  function buildQuickForm() {
    ...
    $this->add('select', 'case_roles_select', ts('Select the case role(s) to which the email will be sent'), $this->getCaseRoles(), TRUE,
      array('id' => 'case_activity_type_ids', 'class' => 'crm-select2', 'multiple' => 'multiple', 'class' => 'crm-select2'));
    if ($this->hasCase) {
  ...
  public function setDefaultValues() {
    ...
    if (!empty($data['case_roles_select'])) {
      $defaultValues['case_roles_select'] = $data['case_roles_select'];
    }
		if (!empty($data['cc'])) {
  ...
  public function postProcess() {
    ...
    $data['case_roles_select'] = $this->_submitValues['case_roles_select'];
		$data['cc'] = '';
